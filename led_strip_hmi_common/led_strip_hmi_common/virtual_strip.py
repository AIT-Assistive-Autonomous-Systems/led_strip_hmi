...
class VirtualStrip:
    ...
    def __init__(self, yaml_path: str) -> None:
        ...
        if not config_path.is_file():
            raise FileNotFoundError(f"Cannot find configuration file: {yaml_path}")
        ...
        for idx, name in enumerate(self.order):
            ...
            next_name = (
                self.order[(idx + 1) % len(self.order)] if self.closed
                else self.order[idx + 1] if idx + 1 < len(self.order) else None
            )
            if next_name:
                end_pt = polygon[-1]
                ...
                if not np.allclose(end_pt, start_next):
                    ...
                    gap_segment.length = float(np.linalg.norm(pts2d[1] - pts2d[0]))
        ...
        self.cum_lengths: List[float] = [0.0] + [seg.start + seg.length for seg in self.segments]
            ...
            for i in range(n):
                ...
                for j, length in enumerate(seg_lengths):
                    if acc + length >= d_req or j == len(seg_lengths) - 1:
                        ...
                        ratio = (strip.start + (i + 0.5) * dled ) / self.total_length
        ...
        for idx, seg in enumerate(self.segments):
            ...
            for s in self.segments:
                ...
                if s.start + s.length <= seg.start:
                    ...
                    break
            strip_len = sum(s.length for s in self.segments if s.strip == seg.strip)
            if strip_len < 1e-8:
                return seg.strip, 0
