...
from .adapters import build_led_strip_config
from .drawing import (
    create_canvas,
    draw_axes,
    draw_real_strips,
...
class ProjectorNode(Node):
    ...
    def __init__(self):
        ...
        cfg_msg = build_led_strip_config(self.cfg, self.get_clock().now())
        ...
        self.sub = self.create_subscription(
            Detection3DArray, 'detection_vision_3d',
            self.cb_detections, 10
        )

        self.sub_scan = self.create_subscription(
            LaserScan, 'scan',
            self.cb_scan, qos_profile=rclpy.qos.qos_profile_sensor_data
        )
    ...
    def _log_configuration(self) -> None:
        ...
        lines = [
            ...
            "",
            " Topics:",
            f"   Sub /detection_vision_3d",
            f"   Pub /led_indices",
            f"   Pub /debug_image",
            f"   Pub /led_strip_config",
            "===================================="
        ]
    ...
    def cb_scan(self, msg: LaserScan):
        ...
        virtual_frame_id = self.cfg.vp_frame
        ...
        for i, dist in enumerate(msg.ranges):
            ...
            # print(pt_v, pt_sensor)
            angle = math.atan2(pt_v[1], pt_v[0])

            # print(angle, angle_sensor)
            ...
            if dangle[lidx] < 0.1:
                nc = led_ratios[lidx]
                # print(nc, angle_sensor, angle)
                # print(dangle[lidx], angle, led_angles[lidx], nc
                norms_list.append((nc, nc, nc, dist))

        # norms_list.append((0.5,0.4,0.6,0.1))
        # norms_list.append((0.25,0.25,0.25,0.2))

        # convert laserscan to
    ...
    def publish_debug_image(self, msg, detection_results) -> None:
        ...
        draw_real_strips(
            im, self.cfg.strips, self.tfbuf,
            self.cfg.strip_frame, self.cfg.vp_frame,
            msg.header.stamp, self.cfg.img_size,
            self.cfg.max_d, logger=self.get_logger()
        )
        draw_virtual_strip(
            im, self.cfg.virtual_strip, self.tfbuf,
            self.cfg.strip_frame, self.cfg.vp_frame,
            msg.header.stamp, self.cfg.img_size,
            self.cfg.max_d, logger=self.get_logger()
        )
        draw_detections(
            im, msg.detections, self.tfbuf,
            self.cfg.strip_frame, self.cfg.vp_frame,
            msg.header.stamp, self.cfg.img_size,
            self.cfg.max_d, self.cfg.center_px,
            logger=self.get_logger()
        )

        # 6) draw the blue dot for each valid centre_ratio
        for ratios, dist in detection_results:
            ...
            p3c = transform_point(
                self.tfbuf, p3, self.cfg.vp_frame,
                Duration(seconds=0.1), logger=self.get_logger()
            )
            if p3c is None:
                continue
            pix = world_to_pixel(
        ...
        # 7) finally, publish the debug image
    ...
    def cb_detections(self, msg: Detection3DArray) -> None:
        ...
        Args:
            msg (Detection3DArray): Array of 3D detections from vision.
        """

        # 3) compute the virtual‐camera origin in the strip frame
        o_cs = PointStamped()
        ...
        cam_strip = transform_point(
            self.tfbuf, o_cs, self.cfg.strip_frame,
            Duration(seconds=0.1), logger=self.get_logger()
        )
        if cam_strip is None:
            return

        # 4) map each Detection3D into strip‐normalized ratios
        detection_results = []
        ...
        # 5) publish LEDStripProjectionInfoArray
        idx_arr = LEDStripProjectionInfoArray()
        idx_arr.header.stamp = msg.header.stamp
        idx_arr.header.frame_id = self.cfg.strip_frame
